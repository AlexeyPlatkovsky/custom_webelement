...
Copy one of the two formats below, fill in every field, then invoke `/ai-write-test` with the file path.
...
- **Auth required** — provide it explicitly, or imply it through the steps ("User logs in as Admin", credentials in test data). The skill will infer it; it will only ask if auth state is unclear.
